...
def rnn_step_backward(dnext_h, cache):
    ...
    # Local gradient of tanh: 1 - tanh(x)^2
...
def rnn_backward(dh, cache):
    ...
    for t in range(T - 1, -1, -1):
        ...
        tdx, dprev_h, tdWx, tdWh, tdb = rnn_step_backward(dnext_h, caches[t])
        dx[:, t, :] = tdx
        dWx += tdWx
        dWh += tdWh
        db += tdb
...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    loss = F.cross_entropy(x.view(-1, x.size(-1)), y.view(-1), ignore_index=ignore_index, reduction='mean')
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        # Projection from CNN features to initial hidden state
        self.feature_proj = nn.Linear(self.image_encoder.out_channels, hidden_dim)
        
        # For attention LSTM, project to spatial features
        if cell_type == "attn":
            self.A_proj = nn.Linear(self.image_encoder.out_channels, hidden_dim)
        
        # Output projection from hidden state to vocab
    ...
    def forward(self, images, captions):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        # Get CNN features from images
        features = self.image_encoder(images)  # (N, C, H/32, W/32)
        N = features.shape[0]
        
        if self.cell_type == "attn":
            # For attention LSTM, project to spatial features (N, H, 4, 4)
            # Reshape: (N, C, 4, 4) -> (N, 4, 4, C) -> (N*16, C)
            features_reshaped = features.permute(0, 2, 3, 1).contiguous().view(-1, features.shape[1])
            A_proj = self.A_proj(features_reshaped)  # (N*16, H)
            A = A_proj.view(N, 4, 4, self.output_proj.in_features)
            A = A.permute(0, 3, 1, 2)  # (N, H, 4, 4)
            h = self.attn_lstm(self.word_embed(captions_in), A)  # (N, T, H)
        else:
            # Pool features to (N, C)
            features_pooled = features.mean(dim=(2, 3))  # (N, C)
            h0 = self.feature_proj(features_pooled)  # (N, H)
            ...
            if self.cell_type == "rnn":
                h = self.rnn(word_vecs, h0)  # (N, T, H)
            else:  # lstm
                h = self.rnn(word_vecs, h0)  # (N, T, H)
        
        # Project hidden states to vocabulary scores
    ...
    def sample(self, images, max_length=15):
        ...
        if self.cell_type == "attn":
            # Project to spatial features (N, H, 4, 4)
            features_reshaped = features.permute(0, 2, 3, 1).contiguous().view(-1, features.shape[1])
            A_proj = self.A_proj(features_reshaped)
            A = A_proj.view(batch_size, 4, 4, self.output_proj.in_features)
            A = A.permute(0, 3, 1, 2)  # (N, H, 4, 4)
            
            h = A.mean(dim=(2, 3))  # Initial hidden state (N, H)
            c = h.clone()  # Initial cell state (N, H)
            attn = h  # Initial attention
        else:
            # Pool features
            features_pooled = features.mean(dim=(2, 3))  # (N, C)
            h = self.feature_proj(features_pooled)  # (N, H)
            
            if self.cell_type == "lstm":
                c = torch.zeros_like(h)  # Initialize cell state for LSTM
        
        # Initialize with START token
        prev_word = torch.full((batch_size,), self._start, dtype=torch.long, device=images.device)
        
        for t in range(max_length):
            # Embed the word
            word_embed = self.word_embed(prev_word)  # (N, W)
            
            # RNN/LSTM step
            if self.cell_type == "rnn":
                h = self.rnn.step_forward(word_embed, h)  # (N, H)
            elif self.cell_type == "lstm":
                h, c = self.rnn.step_forward(word_embed, h, c)  # (N, H)
            elif self.cell_type == "attn":
                attn, attn_w = dot_product_attention(h, A)  # attn: (N, H)
                h, c = self.attn_lstm.step_forward(word_embed, h, c, attn)  # (N, H)
                attn_weights_all[:, t, :, :] = attn_w
            
            # Get scores for all words
            scores = self.output_proj(h)  # (N, V)
            
            # Sample the word with highest score
            prev_word = torch.argmax(scores, dim=1)  # (N,)
...
def dot_product_attention(prev_h, A):
    ...
    # Compute attention: prev_h @ A_flat -> (N, 16)
    attn_logits = torch.bmm(prev_h.unsqueeze(1), A_flat).squeeze(1)  # (N, 16)
    # Scale by sqrt(H)
    attn_logits = attn_logits / math.sqrt(H)
    # Softmax
    attn_weights = torch.softmax(attn_logits, dim=1)  # (N, 16)
    # Reshape back to (N, 4, 4)
    attn_weights = attn_weights.view(N, 4, 4)
    # Compute attention embedding: sum_i attn_weights[i] * A[i]
    attn = (attn_weights.unsqueeze(1) * A).sum(dim=(2, 3))  # (N, H)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
