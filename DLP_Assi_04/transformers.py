...
def generate_token_dict(vocab):
    ...
    ##############################################################################
    token_dict = {val: i for i, val in enumerate(vocab)}
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return token_dict
...
def scaled_dot_product_two_loop_batch(
    ...
    for n in range(N):
        # weights: (K, K)
        weights = torch.zeros(K, K, device=query.device, dtype=query.dtype)
        for i in range(K):
            for j in range(K):
                weights[i, j] = torch.dot(query[n, i], key[n, j]) / math.sqrt(M)
        weights = torch.softmax(weights, dim=1)
        out[n] = torch.bmm(weights.unsqueeze(0), value[n].unsqueeze(0)).squeeze(0)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
class SelfAttention(nn.Module):
    ...
    def forward(
        ...
        k = self.k(key)
        v = self.v(value)
        y, self.weights_softmax = scaled_dot_product_no_loop_batch(q, k, v, mask=mask)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        c = math.sqrt(6.0 / (self.linear.in_features + self.linear.out_features))
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2
        ...
        self.ffb = FeedForwardBlock(emb_dim, feedforward_dim)
    ...
    def forward(self, x):
        ...
        # x shape: (N, K, M)
        out1 = self.mha(x, x, x)
        out2 = self.ln1(x + out1)
        out2 = self.dropout(out2)
        
        out3 = self.ffb(out2)
        y = self.ln2(out2 + out3)
        y = self.dropout(y)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    n = torch.arange(K).float() / K
    y = n.unsqueeze(1).repeat(1, M)
    y = y.unsqueeze(0)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
